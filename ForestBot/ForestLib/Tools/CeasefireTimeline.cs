...
{
    private readonly Age105Settings age;
    private readonly StrategySettings strat;

    public CeasefireTimeline(Age105Settings age, StrategySettings strat)
    {
        this.age = age;
        this.strat = strat;
    }

    public List<TimelineEvent> GetTimelineWithExpedientAndHaste(UtopiaDate warEndDate)
    {
    ...
    public List<TimelineEvent> GetTimelineWithExpedient(UtopiaDate warEndDate)
    {
        var ret = new List<TimelineEvent>();
        ret.Add(new TimelineEvent(warEndDate, "War ended"));
        UtopiaDate cfEnd = warEndDate.AddTicks(age.CeasefireLength + 1);
